...
def flood(target):
    ...
    try:
        ...
        print(
            f"{Fore.MAGENTA}Ошибка отправки поддельного UDP пакета\n{Fore.MAGENTA}{e}{Fore.RESET}"
        )
    else:
        print(
            f"{Fore.GREEN}[+] {Fore.YELLOW}Отправка {packets} поддельных UDP пакетов с сервера {server} на {'{}:{}'.format(*target)}.{Fore.RESET}"
        )
