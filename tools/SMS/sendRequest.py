...
class Service:
    ...
    def sendMessage(self, phone):
        ...
        # Create suffixes
        okay = f"{Fore.YELLOW}Сервис ({getDomain(url)}) >> Отправил!{Fore.RESET}"
        error = f"{Fore.MAGENTA}Сервис ({getDomain(url)}) >> Не отправил!{Fore.RESET}"
        ...
        try:
            ...
            print(f"{Fore.RED}[ЗАЕБАЛ ЖДАТЬ] {error}")
            ...
            if r.status_code == 200:
                print(f"{Fore.GREEN}[ЗАЕБИСЬ] {okay}")
            elif r.status_code == 429:
                print(f"{Fore.RED}[ОВЕРДОХУЯ ЗАПРОСОВ] {error}")
            else:
                print(f"{Fore.RED}[БЛЯТЬ] {error}")
