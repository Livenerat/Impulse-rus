    ...
    try:
        ...
        for i in range(len(ipv4)):
            if ipaddress.ip_address(origin) in ipaddress.ip_network(ipv4[i]):
                print(
                    f"{Fore.RED}[!] {Fore.YELLOW}Этот сайт защищён CloudFlare, атака бесполезна.{Fore.RESET}"
                )
                sleep(1)
...
def __GetAddressInfo(target):
    try:
        ...
        print(f"{Fore.RED}[!] {Fore.MAGENTA}Введи ip и порт{Fore.RESET}")
        sys.exit(1)
    else:
        return ip, port
...
def __GetEmailMessage():
    ...
    subject = input(f"{Fore.BLUE}[?] {Fore.MAGENTA}Введи тему (оставь пустым на рандом): ")
    body = input(f"{Fore.BLUE}[?] {Fore.MAGENTA}Введи сообщение (оставь пустым на рандом): ")
...
def InternetConnectionCheck():
    try:
        ...
        print(
            f"{Fore.RED}[!] {Fore.MAGENTA}Устройство не подключено к интернету{Fore.RESET}"
        )
        sys.exit(1)
